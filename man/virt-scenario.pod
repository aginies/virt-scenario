=encoding utf8

=head1 NAME

virt-scenario - Create XML config and prepare host for a specific scenario

=head1 SYNOPSIS

B<virt-scenario>

=head1 DESCRIPTION

B<virt-scenario> prepare a libvirt XML guest configuration and the host to run a customized guest.
Idea is to use multiple B<templates> and concatenate them to create the
expected Guest XML file. If Host need a custom setting it will be done in second phase.

Customization to match a specific scenario is not graved in stone. The idea is to
prepare a configuration which should improved the usage compared to a basic setting.
This will B<NOT guarantee> that this is perfect.

B<virt-scenario> is only available in interactive mode.

=head1 CONFIGURATION

User can set some parameter in the B</etc/virtscenario.yaml>. This will
override the scenario setting (but will display the recommended setting).

C<# WARNING: INCORRET PARAMATERS WILL LEAD TO BAD VM CONFIGURATION>
C<# Dont change the section name>
C<# This will overwrite scenario settings....>
C<emulator:>
C<  - emulator: /usr/bin/qemu-system-x86_64>
C<input:>
C<  - keyboard: virtio>
C<  - mouse: virtio>
C<architecture:>
C<  - arch: x86_64>
C<STORAGE_DATA:>
C<# some options are only available with qcow2 format and>
C<# will be ignored in case of any other image format>
C<  - disk_type: file>
C<#  - disk_cache: none>
C<  - disk_target: vda>
C<  - disk_bus: virtio>
C<  - path: /var/livirt/images>
C<  - format: qcow2>
C<# host side: qemu-img creation options (-o), qemu-img --help>
C<  - unit: G>
C<  - capacity: 20>
C<  - cluster_size: 2M>
C<  - lazy_refcounts: on>
C<# preallocation: off, metadata (qcow2), falloc, full>
C<  - preallocation: off>
C<  - compression_type: zlib>
C<  - encryption: off>

=head1 TEMPLATES DEFINITION

All templates are in python lib virt-scenario directory.

=head1 POSSIBLE SCENARIOS

=over 3

=item computation

=item Desktop

=item Secure VM

=back

=head1 Interactive command

=over 3

=item B<name>: define a name for the VM

=item B<vcpu>: Choose how many VCPU

=item B<memory>: Choose the Memory size (in GiB)

=item B<machine>: Select the Machine type (from a list)

=item B<bootdev>: Select the boot dev (from a list)

=item B<diskpath>: Directory where to store disk image

=item B<conf>: Path to disk image

=item B<computation>: Create an XML configuration to do computation VM

=item B<desktop>: Create an XML configuration for Desktop Usage

=item B<securevm>: create an XML configuration to support Secure VM 

=item B<shell>: execution of a system command

=back

=head1 AUTHORS

Written by Antoine Ginies

=cut
